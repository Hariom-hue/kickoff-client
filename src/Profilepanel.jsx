    ...
    <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
      {vals.map((d, i) => (
        <input key={i} ref={refs[i]} type="text" inputMode="numeric" maxLength={1} value={d}
          ...
          style={{ width: 44, height: 54, textAlign: "center", background: "#0c0c0c", border: `2px solid ${d ? "#c8ff00" : "#252525"}`, borderRadius: 10, color: "#fff", fontSize: 20, fontWeight: 700, fontFamily: "'DM Sans', sans-serif", outline: "none", transition: "border-color 0.2s", cursor: "text" }} />
      ))}
    </div>
  );
}
      ...
      <div style={{ display: "flex", alignItems: "flex-start", position: "relative" }}>
        ...
        {stages.map((s, i) => (
          <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, zIndex: 2, flex: 1 }}>
            ...
            <p style={{ color: i <= currentStage ? "#c8ff00" : "#333", fontSize: 7.5, fontWeight: 700, textAlign: "center", whiteSpace: "nowrap" }}>{s}</p>
          </div>
        ))}
      </div>
    </div>
...
export default function ProfilePanel({ user, setUser, setShowProfile, setAuthMode, authMode }) {
  /* ── auth flow ── */
  const [flow,     setFlow]     = useState("idle");      // idle | signup_email | signup_otp | login_email | login_otp
  ...
  /* ── profile tabs ── */
  const [tab, setTab] = useState("profile"); // profile | orders | addresses | payment | notifications
  ...
  const primaryBtn = (disabled) => ({ width: "100%", padding: "13px", borderRadius: 10, background: disabled ? "#1a1a1a" : "#c8ff00", color: disabled ? "#444" : "#0a0a0a", fontWeight: 700, fontSize: 13, letterSpacing: "0.08em", textTransform: "uppercase", border: "none", cursor: disabled ? "not-allowed" : "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.15s" });
  ...
  /* ── load profile + orders when user logs in ── */
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNotifEnabled(data.notifications !== false);
      }
    } catch {}
  }, [user]);
  ...
  useEffect(() => {
    if (user && tab === "orders") fetchOrders();
  ...
  const sendSignupOTP = async () => {
    ...
    } catch { setOtpErr("Server error. Is the server running?"); }
    setSending(false);
  };
  ...
  const verifySignupOTP = async (code) => {
    ...
    try {
      ...
      if (!res.ok) { setOtpErr(data.error || "Invalid OTP"); }
        ...
        setShowProfile(false); setFlow("idle");
      }
  ...
  const saveInfo = async () => {
    ...
    try {
      ...
      setProfile(p => ({ ...p, name: editName, phone: editPhone }));
  ...
  return (
    <div className="fixed inset-0 z-50" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .pp-toggle{transition:background 0.2s;}
        .pp-tab:hover{color:#c8ff00!important;}
      `}</style>

      {/* Backdrop */}
      <div className="absolute inset-0" style={{ background: "rgba(0,0,0,0.82)", backdropFilter: "blur(8px)" }}
        onClick={() => { setShowProfile(false); if (setAuthMode) setAuthMode(null); }} />
          ...
          {user && (
            <div style={{ display: "flex", gap: 0, overflowX: "auto", scrollbarWidth: "none" }}>
              {[
                ...
                <button key={t.key} onClick={() => setTab(t.key)} className="pp-tab"
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "22px 28px" }}>
          ...
          {!user && flow === "idle" && (
            <div className="pp-fade" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <button onClick={() => { setFlow("login_email"); setOtpErr(""); }}
                style={{ ...primaryBtn(false), background: "transparent", color: "#fff", border: "1px solid #2a2a2a" }}>
                Sign In
              </button>
            </div>
          )}
          ...
          {!user && flow === "signup_email" && (
            <div className="pp-fade" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div>
                ...
                <input type="email" className="pp-inp" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@gmail.com" style={inp}
                  onFocus={e=>e.target.style.borderColor="#c8ff00"} onBlur={e=>e.target.style.borderColor="#2a2a2a"} />
              </div>
              <div>
                <label style={lbl}>Password</label>
                <input type="password" className="pp-inp" value={password} onChange={e => setPassword(e.target.value)} placeholder="Min 6 characters" style={inp}
                  onFocus={e=>e.target.style.borderColor="#c8ff00"} onBlur={e=>e.target.style.borderColor="#2a2a2a"}
                  onKeyDown={e => e.key === "Enter" && sendSignupOTP()} />
              </div>
              {otpErr && <p style={{ color: "#ef4444", fontSize: 12, margin: 0 }}>⚠️ {otpErr}</p>}
              <button onClick={sendSignupOTP} disabled={sending} style={primaryBtn(sending)}>
                {sending ? <><span className="pp-spin">⟳</span> Sending OTP...</> : "Send OTP to Gmail →"}
              </button>
              <p style={{ textAlign: "center", color: "#444", fontSize: 12 }}>
                Have an account? <button onClick={() => { setFlow("login_email"); setOtpErr(""); }} style={{ background: "none", border: "none", color: "#c8ff00", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Sign in</button>
              </p>
            </div>
          )}
          ...
          {!user && flow === "signup_otp" && (
            <div className="pp-fade" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <button onClick={() => verifySignupOTP()} disabled={verifying || otp.length < 6} style={primaryBtn(verifying || otp.length < 6)}>
                {verifying ? <><span className="pp-spin">⟳</span> Verifying...</> : "Verify & Create Account →"}
              </button>
              <p style={{ textAlign: "center", color: "#444", fontSize: 12, margin: 0 }}>
                {resendIn > 0
                  ? <span style={{ color: "#333" }}>Resend in {resendIn}s</span>
                  : <button onClick={sendSignupOTP} style={{ background: "none", border: "none", color: "#c8ff00", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Resend OTP</button>
                }
              </p>
          ...
          {!user && flow === "login_email" && (
            <div className="pp-fade" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {otpErr && <p style={{ color: "#ef4444", fontSize: 12, margin: 0 }}>⚠️ {otpErr}</p>}
              <button onClick={sendLoginOTP} disabled={sending} style={primaryBtn(sending)}>
                {sending ? <><span className="pp-spin">⟳</span> Sending OTP...</> : "Send OTP to Gmail →"}
              </button>
              <p style={{ textAlign: "center", color: "#444", fontSize: 12 }}>
                No account? <button onClick={() => { setFlow("signup_email"); setOtpErr(""); }} style={{ background: "none", border: "none", color: "#c8ff00", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Create one</button>
              </p>
            </div>
          )}
          ...
          {!user && flow === "login_otp" && (
            <div className="pp-fade" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {otpErr && <p style={{ color: "#ef4444", fontSize: 12, margin: 0, textAlign: "center" }}>⚠️ {otpErr}</p>}
              <button onClick={() => verifyLoginOTP()} disabled={verifying || otp.length < 6} style={primaryBtn(verifying || otp.length < 6)}>
                {verifying ? <><span className="pp-spin">⟳</span> Verifying...</> : "Sign In →"}
              </button>
              <p style={{ textAlign: "center", color: "#444", fontSize: 12, margin: 0 }}>
                {resendIn > 0
                  ? <span style={{ color: "#333" }}>Resend in {resendIn}s</span>
                  : <button onClick={sendLoginOTP} style={{ background: "none", border: "none", color: "#c8ff00", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Resend OTP</button>
                }
              </p>
            </div>
          )}
            ...
            <div className="pp-fade" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {[
                ...
                <button key={i} onClick={item.onClick}
                  style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 14px", background: "#111", border: "1px solid #1e1e1e", borderRadius: 12, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", width: "100%", textAlign: "left", transition: "border-color 0.2s" }}
                  onMouseEnter={e=>e.currentTarget.style.borderColor="#2a2a2a"} onMouseLeave={e=>e.currentTarget.style.borderColor="#1e1e1e"}>
            ...
            <div className="pp-fade" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {(profile?.savedAddresses || []).map((addr, i) => (
                <div key={i} style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 12, padding: "12px 14px", display: "flex", gap: 10 }}>
                  ...
                  <button onClick={() => deleteAddress(i)} style={{ background: "none", border: "none", color: "#ff5555", fontSize: 12, cursor: "pointer", alignSelf: "flex-start", padding: "2px 4px" }}>✕</button>
                </div>
              ))}
              ...
              {addingAddr ? (
                <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 12, padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  {[
                    ...
                    <div key={f.key}>
                      ...
                      <input className="pp-inp" value={newAddr[f.key]} onChange={e => setNewAddr(a => ({ ...a, [f.key]: e.target.value }))} placeholder={f.placeholder} style={inp}
                        onFocus={e=>e.target.style.borderColor="#c8ff00"} onBlur={e=>e.target.style.borderColor="#2a2a2a"} />
                    </div>
                  ))}
                  <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={() => setAddingAddr(true)} style={{ ...primaryBtn(false), padding: "11px" }}>+ Add New Address</button>
              )}
            </div>
          )}
          ...
          {user && tab === "notifications" && (
            <div className="pp-fade" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", background: "#111", border: "1px solid #1e1e1e", borderRadius: 12 }}>
                ...
                <button onClick={toggleNotifications} disabled={savingNotif} className="pp-toggle"
                  style={{ width: 44, height: 24, borderRadius: 12, background: notifEnabled ? "#c8ff00" : "#2a2a2a", border: "none", cursor: "pointer", position: "relative", flexShrink: 0 }}>
              ...
              {/* Notification types */}
              {[
                { icon: "📦", label: "Order Updates",     sub: "Shipping, delivery & tracking",            enabled: notifEnabled },
                { icon: "🏷️", label: "Offers & Deals",    sub: "Discounts and promotional codes",          enabled: notifEnabled },
                { icon: "🔔", label: "New Arrivals",       sub: "Latest jersey drops from your leagues",    enabled: notifEnabled },
                { icon: "⭐", label: "Review Reminders",   sub: "Rate your recent purchases",               enabled: notifEnabled },
              ].map((n, i) => (
              ...
              <p style={{ color: "#333", fontSize: 11, textAlign: "center", margin: 0 }}>
                Notifications are sent to <span style={{ color: "#444" }}>{user}</span>
              </p>
            </div>
          )}
