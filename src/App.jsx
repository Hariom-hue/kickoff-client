// MOBILE-OPTIMIZED APP.JSX - All horizontal scroll issues fixed
...
function CartFlyAnimation({ item, origin, onDone }) {
  ...
  useEffect(() => {
    ...
    const target = cartEl ? cartEl.getBoundingClientRect() : { left: window.innerWidth - 60, top: 20, width: 42, height: 42 };
  ...
  return (
    <div ref={ref} style={{ position: "fixed", left: origin.x - 28, top: origin.y - 28, width: 56, height: 56, borderRadius: 10, overflow: "hidden", zIndex: 9999, pointerEvents: "none", border: "2px solid #c8ff00", boxShadow: "0 0 20px rgba(200,255,0,0.6)" }}>
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="absolute right-0 top-0 bottom-0 w-full max-w-md flex flex-col" style={{ background: "#0c0c0c", borderLeft: "1px solid #1e1e1e" }}>
        <div className="flex items-center justify-between px-6 sm:px-8 py-6" style={{ borderBottom: "1px solid #1e1e1e" }}>
          <div>
...
function Navbar({ search, setSearch, cart, setShowCart, setShowProfile, setShowSidebar, onLogoClick, user, cartBounce }) {
  return (
    <>
      ...
      `}</style>
      {/* 🔧 FIXED: Responsive padding - 16px on mobile, 28px on desktop */}
      <header style={{ background: "#0a0a0a", borderBottom: "1px solid #181818", position: "sticky", top: 0, zIndex: 40, fontFamily: "'DM Sans', sans-serif" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "0 16px", height: 64 }} className="sm:gap-16 sm:px-28">
          ...
          <h1 onClick={onLogoClick} style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "1.5rem", letterSpacing: "0.12em", cursor: "pointer", flexShrink: 0, lineHeight: 1, marginRight: 8 }} className="sm:text-[1.7rem]">
            <span style={{ color: "#fff" }}>KICK</span><span style={{ color: "#c8ff00" }}>OFF</span>
          </h1>
          {/* 🔧 FIXED: Added min-w-0 to allow shrinking */}
          <div style={{ flex: 1, position: "relative", maxWidth: 440, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: "auto", flexShrink: 0 }} className="sm:gap-12">
            <button onClick={() => setShowProfile(true)}
              ...
              <span style={{ color: user ? "#fff" : "#666", fontSize: 12, fontWeight: 600 }} className="hidden sm:inline">{user ? user.split("@")[0] : "Sign in"}</span>
            </button>
...
function Footer({ leagues, goLeague }) {
  return (
    <footer style={{ background: "#060606", borderTop: "1px solid #161616", fontFamily: "'DM Sans', sans-serif", marginTop: 80 }}>
      ...
      <div style={{ borderBottom: "1px solid #111", padding: "24px 16px" }} className="sm:px-40">
        <div style={{ maxWidth: 1400, margin: "0 auto", display: "flex", flexWrap: "wrap", gap: 20, alignItems: "center", justifyContent: "center" }} className="sm:gap-28">
          {[
      ...
      {/* 🔧 FIXED: Responsive grid - 1 col mobile, 4 cols desktop */}
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "44px 16px 32px", display: "grid", gridTemplateColumns: "1fr", gap: 32 }} className="sm:px-40 md:grid-cols-2 lg:grid-cols-4 md:gap-40 footer-grid">
        <div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            {[{l:"📸",h:"https://instagram.com"},{l:"🐦",h:"https://twitter.com"},{l:"▶️",h:"https://youtube.com"}].map((s,i)=>(
              <a key={i} href={s.h} target="_blank" rel="noreferrer" style={{ width:36,height:36,borderRadius:8,background:"#111",border:"1px solid #222",display:"flex",alignItems:"center",justifyContent:"center",textDecoration:"none",fontSize:14 }}
        ...
        <div>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:11 }}>
            {leagues.slice(0,6).map((l,i)=>(
              <button key={i} onClick={()=>goLeague(l)} style={{ background:"none",border:"none",color:"#555",fontSize:13,cursor:"pointer",textAlign:"left",padding:0,fontFamily:"'DM Sans',sans-serif" }}
        ...
        <div>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:11 }}>
            {["Home","Catalog","New Arrivals","Sale","Track Order"].map(link=>(
              <a key={link} href="#" style={{ color:"#555",fontSize:13,textDecoration:"none" }}
        ...
        <div>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:11 }}>
            {["Privacy Policy","Refund Policy","Terms of Use","Shipping Policy","Contact Us","About Us"].map(link=>(
              <a key={link} href="#" style={{ color:"#555",fontSize:13,textDecoration:"none" }}
                onMouseEnter={e=>e.currentTarget.style.color="#fff"} onMouseLeave={e=>e.currentTarget.style.color="#555"}>{link}</a>
            ))}
          </div>
      ...
      {/* 🔧 FIXED: Responsive padding */}
      <div style={{ borderTop:"1px solid #111",padding:"22px 16px" }} className="sm:px-40">
        <div style={{ maxWidth:1400,margin:"0 auto",display:"flex",flexWrap:"wrap",alignItems:"center",justifyContent:"space-between",gap:20 }}>
          <div style={{ display:"flex",alignItems:"center",gap:16,flexWrap:"wrap" }} className="sm:gap-20">
            <div>
            ...
            <div style={{ width:1,height:36,background:"#1e1e1e" }} className="hidden sm:block" />
            <div style={{ display:"flex",alignItems:"center",gap:8 }}>
          ...
          <div style={{ display:"flex",gap:10,flexWrap:"wrap",justifyContent:"center" }} className="sm:gap-14">
            {[
      ...
      {/* 🔧 FIXED: Responsive padding */}
      <div style={{ borderTop:"1px solid #0d0d0d",padding:"16px 16px",display:"flex",flexWrap:"wrap",alignItems:"center",justifyContent:"space-between",gap:12 }} className="sm:px-40">
...
// Just wrap the return in an overflow-x-hidden container:
...
function App() {
    ...
    return (
      <>
        <ProductDetail
          ...
          Navbar={<Navbar {...navProps} />}
        />
        {showProfile && <ProfilePanel user={user} setUser={setUser} setShowProfile={setShowProfile} authMode={authMode} setAuthMode={setAuthMode} />}
      </>
    );
  }
  ...
  if (view === "league" && selectedLeague) {
    ...
    return (
      ...
      <div className="overflow-x-hidden" style={{ background: "#0a0a0a", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
        <style>{`@import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@300;400;500;600;700&display=swap');
          .pc:hover{border-color:#c8ff00!important;transform:translateY(-3px);} .pc{transition:border-color 0.25s,transform 0.25s;}
          .pc-img{transition:transform 0.4s;} .pc:hover .pc-img{transform:scale(1.07);}
        ...
        <Navbar {...navProps} />
        {showSidebar && <Sidebar setShowSidebar={setShowSidebar} goHome={goHome} leagues={leagues} goLeague={goLeague} selectedLeague={selectedLeague} />}
        {/* 🔧 FIXED: Responsive padding */}
        <div style={{ background: "#111", borderBottom: "1px solid #1e1e1e", padding: "28px 16px 24px" }} className="sm:px-28">
          ...
          <div style={{ display:"flex",alignItems:"center",gap:20,flexWrap:"wrap" }}>
            ...
            <span style={{ fontSize:12,color:"#444",flexShrink:0 }}>{filtered.length} jersey{filtered.length!==1?"s":""}</span>
          </div>
        </div>
        {/* 🔧 FIXED: Responsive padding */}
        <div style={{ maxWidth:1400,margin:"0 auto",padding:"32px 16px" }} className="sm:px-28">
          {loadingProducts ? (
            ...
            // 🔧 FIXED: Smaller minimum card width for mobile
            <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(160px, 1fr))",gap:18 }} className="sm:grid-cols-[repeat(auto-fill,minmax(240px,1fr))]">
  ...
  return (
    ...
    <div className="overflow-x-hidden" style={{ background: "#0a0a0a", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .pc{transition:border-color 0.25s,transform 0.25s;} .pc:hover{border-color:#c8ff00!important;transform:translateY(-3px);}
        .pc-img{transition:transform 0.4s;} .pc:hover .pc-img{transform:scale(1.07);}
        .addbtn{transition:background 0.2s,color 0.2s,border-color 0.2s;}
        .addbtn:hover{background:#c8ff00!important;color:#0a0a0a!important;border-color:#c8ff00!important;}
      `}</style>
      {showSidebar && <Sidebar setShowSidebar={setShowSidebar} goHome={goHome} leagues={leagues} goLeague={goLeague} selectedLeague={null} />}
      <Navbar {...navProps} />
      {/* 🔧 FIXED: Responsive padding */}
      <main style={{ maxWidth:1400,margin:"0 auto",padding:"44px 16px" }} className="sm:px-28">
        {search.trim().length > 1 ? (
          <div>
            ...
            {displayProducts.length === 0 ? <p style={{ color:"#444",fontSize:14 }}>No products found.</p> : (
              // 🔧 FIXED: Smaller minimum card width
              <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(160px, 1fr))",gap:18 }} className="sm:grid-cols-[repeat(auto-fill,minmax(240px,1fr))]">
          ...
          <>
            ...
            {loadingLeagues ? (
              ...
              // 🔧 FIXED: Smaller minimum width for league cards
              <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(140px, 1fr))",gap:14,marginBottom:56 }} className="sm:grid-cols-[repeat(auto-fill,minmax(180px,1fr))]">
                {leagues.map((league,i)=>(
                  <button key={i} className="lc" onClick={()=>goLeague(league)}
                    style={{ background:"#111",border:"1px solid #1e1e1e",borderRadius:16,padding:"20px 12px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:12,textAlign:"center",fontFamily:"'DM Sans',sans-serif" }} className="sm:p-6 sm:gap-14">
                    <div style={{ width:56,height:56,borderRadius:14,background:"#1a1a1a",border:"1px solid #252525",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden" }} className="sm:w-16 sm:h-16">
                      {league.logo ? <img src={league.logo} alt={league.name} style={{ width:"100%",height:"100%",objectFit:"contain" }} /> : <span style={{ fontSize:24 }} className="sm:text-[28px]">{sportEmoji[league.sport]||sportEmoji.default}</span>}
                    </div>
                    <div>
                      <p style={{ color:"#fff",fontWeight:700,fontSize:13,marginBottom:4,lineHeight:1.3 }} className="sm:text-sm">{league.name}</p>
                      <p style={{ color:"#444",fontSize:10,fontWeight:600 }} className="sm:text-[11px]">{league.sport}</p>
                    </div>
                    <div style={{ marginTop:"auto",padding:"4px 10px",background:"rgba(200,255,0,0.06)",border:"1px solid rgba(200,255,0,0.15)",borderRadius:20 }} className="sm:px-14 sm:py-[5px]">
                      <span style={{ color:"#c8ff00",fontSize:9,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase" }} className="sm:text-[10px]">View Jerseys →</span>
                    </div>
                  </button>
                ))}
              </div>
...
function Sidebar({ setShowSidebar, goHome, leagues, goLeague, selectedLeague }) {
  return (
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:2 }}>
          {leagues.map((league,i)=>(
            <button key={i} onClick={()=>{goLeague(league);setShowSidebar(false);}}
              ...
              <div className="flex-1 min-w-0">
                <p style={{ color:selectedLeague?._id===league._id?"#c8ff00":"#ccc",fontWeight:600,fontSize:13,margin:0,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{league.name}</p>
...
function ProductCard({ item, onClick, onAddToCart }) {
  return (
    <div className="pc" onClick={onClick} style={{ background:"#111",border:"1px solid #1a1a1a",borderRadius:18,overflow:"hidden",cursor:"pointer" }}>
      <div style={{ position:"relative",height:200,overflow:"hidden",background:"#161616" }} className="sm:h-60">
        ...
        {item.league && (
          <span style={{ position:"absolute",top:8,left:8,background:"rgba(10,10,10,0.85)",backdropFilter:"blur(4px)",color:"#c8ff00",fontSize:8,fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",padding:"3px 8px",borderRadius:6,border:"1px solid rgba(200,255,0,0.2)" }} className="sm:top-12 sm:left-12 sm:text-[9px] sm:px-10 sm:py-1">
            {item.league}
          </span>
        )}
      </div>
      <div style={{ padding:"14px 16px" }} className="sm:p-[18px]">
        <h3 style={{ color:"#fff",fontWeight:700,fontSize:13,marginBottom:6,lineHeight:1.4,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden" }} className="sm:text-sm">{item.name}</h3>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12 }} className="sm:mb-14">
          <span style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.3rem",color:"#c8ff00",letterSpacing:"0.04em" }} className="sm:text-2xl">₹{item.price}</span>
          {item.category && <span style={{ fontSize:8,fontWeight:700,color:"#444",letterSpacing:"0.1em",textTransform:"uppercase" }} className="sm:text-[9px]">{item.category}</span>}
        </div>
        <button className="addbtn" onClick={onAddToCart}
          style={{ width:"100%",padding:"9px",background:"#161616",border:"1px solid #2a2a2a",borderRadius:9,color:"#fff",fontWeight:700,fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",fontFamily:"'DM Sans',sans-serif" }} className="sm:text-[11px] sm:p-[10px]">
          + Add to Cart
        </button>
      </div>
    </div>
